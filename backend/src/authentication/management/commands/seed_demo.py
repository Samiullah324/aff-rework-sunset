...
environment variable. Safe to re-run: existing users are left as-is; the demo
user's password is (re)synced each run so the printed credential always works.
...
class Command(BaseCommand):
    help = "Seed idempotent demo data: a demo admin login user."
